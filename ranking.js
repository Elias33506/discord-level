  ...
  client.on("message", async (message) => {
    ...
    if (client.points.get(key, `level`) < curLevel) {
      const embed = new Discord.MessageEmbed()
        .setTitle(`Rank von:  ${message.author.username}`)
        .setTimestamp()
        .setDescription(`Du bist auf Level aufgestiegen: **\`${curLevel}\`**! (Punkte: \`${Math.floor(client.points.get(key, `points`) * 100) / 100}\`) `)
        .setColor("GREEN");
    ...
    if (message.content.toLowerCase().startsWith(`${config.PREFIX}rank`)) {
      ...
      if (client.points.get(key, `level`) === undefined) i = `Kein Rank gefunden`;
      let tempmsg = await message.channel.send(new Discord.MessageEmbed().setColor("RED").setAuthor("Bitte warte...", "https://cdn.discordapp.com/emojis/769935094285860894.gif"))
      let color;
      ...
      rank.build()
        .then(async data => {
          ...
          const embed = new Discord.MessageEmbed()
            .setTitle(`Rank von:  ${rankuser.username}`)
            .setColor(color)
    ...
    if (message.content.toLowerCase() === `${config.PREFIX}leaderboard` || message.content.toLowerCase() === `${config.PREFIX}lb`) {
      message.delete()
      const filtered = client.points.filter(p => p.guild === message.guild.id).array();
      const sorted = filtered.sort((a, b) => b.points - a.points);
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .setTimestamp()
        .setDescription(`Top 10 der aktivesten Mitglieder:`)
        .setColor("ORANGE");
      let i = 0;
      for (const data of top10) {
        await delay(15); try {
          i++;
          embed.addField(`**${i}**. ${client.users.cache.get(data.user).tag}`, `Punkte: \`${Math.floor(data.points * 100) / 100}\` | Level: \`${data.level}\``);
        } catch {
          i++; 
          embed.addField(`**${i}**. ${client.users.cache.get(data.user)}`, `Punkte: \`${Math.floor(data.points * 100) / 100}\` | Level: \`${data.level}\``);
        }
      }
      return message.channel.send(embed);
    }

   if (message.content.toLowerCase() === `${config.PREFIX}help` || message.content.toLowerCase() === `${config.PREFIX}hilfe`) {
      message.delete()
      const embed = new Discord.MessageEmbed()
        .setTitle(`${message.guild.name}: Hilfe`)
        .setTimestamp()
        .setDescription("`.help` -> `Das`\n`.rank` -> `Zeigt den Rank von dir oder den Rang anderer Mitglieder an`\n`.leaderboard` -> `Zeigt die 10 aktivesten Mitglieder an`")
        .setColor("ORANGE");
      return message.channel.send(embed);
    }
